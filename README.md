...
4. A WebSocket relay for WebTorrent-style peers (`/tracker/*`): a minimal
   ...
   between two authenticated peers passes through the Worker. Workers cannot
   terminate raw inbound TCP/TLS, only a WebSocket (via HTTP Upgrade), so the
   relay is a plain WebSocket connection into a Durable Object that forwards
   frames between the two peers in a swarm — giving exact, synchronous
   control over the byte limits (§ Tracker below) with no reliance on
   external analytics.
   ...
   connections, which the Workers runtime does not support. `/tracker/*`
   instead implements a small, hand-rolled announce+relay wire protocol
   inspired by the WebTorrent tracker WebSocket protocol.
...
migrations/
  ...
  0002_usage_daily.sql      daily byte-quota ledger for the tracker relay
```
